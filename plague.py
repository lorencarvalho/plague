...
class Infection(object):
    def __init__(self, host, port, message, seed_nodes=[]):
        """
        ...
        # intitialize a infection with no nodes infected
        ...
        self.susceptible_nodes = set(seed_nodes)
...
class PlagueNode(object):
    ...
    def __init__(self, host, port, contaminate=[], loop=None):
        self.host = host
        self.port = port
        self.loop = loop or asyncio.get_event_loop()
        self.susceptible_nodes = set(contaminate)
    ...
    async def _infect(self, message):
        try:
            ...
            log.warning("No susceptible nodes available!")
            return
...
class BackgroundPlagueNode(threading.Thread):
    def __init__(self, port, contaminate=[], loop=None, *args, **kwargs):
        ...
        super(BackgroundPlagueNode, self).__init__(*args, **kwargs)
    ...
    def stop(self):
        ...
        super(BackgroundPlagueNode, self).join()
